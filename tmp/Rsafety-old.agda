...
module project.Resources.Rsafety-old
...
Rsafety (Tlet {t1 = t1} {t2 = t2} ht hti) (Elet {t1' = t1'} ev)
    = lemma↭3 (R t2) (RI ev) (RU ev) (Rsafety ht ev)
Rsafety (Tlet {t1 = t1} {t2 = t2} ht hti) (Elet2 v)
    =
    begin
...
Rsafety (Tpack htv) (Epack ev) = Rsafety-vec htv ev
Rsafety (Tpack {ts = ts} htv) (Epacked {M = M} {s = s} k x) with tyIsLin (Tst (sId M s))
... | yes yLin = refl
... | no  nLin = refl
Rsafety (Tcall htv) (Ecall ev) = Rsafety-vec htv ev
...
Rsafety (Tcall {M2 = M2} {f = f} {ts = ts} htv) (Ecalled vs)
    = begin
    Rv ts               ↭⟨ refl ⟩
    L.[] L.++ Rv ts
...
Rsafety (Tunpack {t1 = t1} {t2 = t2} ht hti) (Eunpack ev)
    = lemma↭3 (R t2) (RI ev) (RU ev) (Rsafety ht ev)
Rsafety (Tunpack (Tstruct _ htv) hti) (Eunpacked {M = M} {t2 = t2} {k = k} {s = s} {ts = ts} vs)
...
Rsafety (Texec ht) (Eexec ev) = Rsafety ht ev
Rsafety (Texec ht) (Eexecuted v) = refl
Rsafety (Tif {t2 = t2} ht1 ht2 ht3) (Eif ev) = lemma↭3 (R t2) (RI ev) (RU ev) (Rsafety ht1 ev)
Rsafety (Tif ht1 ht2 ht3) (Eiftrue nz) = refl

-- TODO : Prove that R t2 ≡ R t3 in if t1 then t2 else t3
Rsafety (Tif {t3 = t3} ht1 ht2 ht3) Eiffalse = {!   !}

Rsafety (TselV {j = j} v (Tstruct _ htv) nLin) (Eselect vs)
    rewrite RnLin (htvLookup vs htv j) nLin (vLookup j vs) = refl
Rsafety (Tpub ht yLin) (Epub ev) = Rsafety ht ev
Rsafety (Tpub {t = t} ht yLin) (Epub2 v) rewrite l++[]≡l (R t) = refl
     
Rsafety-vec (_T∷_ {ts = ts} ht htv) (E[ ev ] vs) = lemma↭3 (Rv ts) (RI ev) (RU ev) (Rsafety ht ev)
Rsafety-vec (ht T∷ htv) (_E∷_ {ts = ts} {ts' = ts'} t evs) =
    begin
    RIv evs L.++ (R t L.++ Rv ts)   ↭⟨ ++⁺ˡ (RIv evs) (++-comm (R t) (Rv ts)) ⟩
    RIv evs L.++ (Rv ts L.++ R t)   ↭⟨ ↭-sym (++-assoc (RIv evs) (Rv ts) (R t)) ⟩
    (RIv evs L.++ Rv ts) L.++ R t   ↭⟨ ++⁺ʳ (R t) (Rsafety-vec htv evs) ⟩
    (RUv evs L.++ Rv ts') L.++ R t  ↭⟨ ++-assoc (RUv evs) (Rv ts') (R t) ⟩
    RUv evs L.++ (Rv ts' L.++ R t)  ↭⟨ ++⁺ˡ (RUv evs) (++-comm (Rv ts') (R t)) ⟩
    RUv evs L.++ (R t L.++ Rv ts')  ∎
    
    -- begin
    -- (R t L.++ Rv ts) L.++ RIv evs   ↭⟨ ++-assoc (R t) (Rv ts) (RIv evs) ⟩
    -- R t L.++ (Rv ts L.++ RIv evs)   ↭⟨ ++⁺ˡ (R t) (Rsafety-vec htv evs) ⟩
    -- R t L.++ (Rv ts' L.++ RUv evs)  ↭⟨ ↭-sym (++-assoc (R t) (Rv ts') (RUv evs)) ⟩
    -- (R t L.++ Rv ts') L.++ RUv evs  ∎
